...
class Laporan_perolehan_nilai extends CI_Controller {
    ...
    $this->load->view('laporan/laporan_perolehan_nilai');
  ...
  public function cetak()
  {
    ...
    $data_kompetisi = $this->db->get_where('ms_kompetisi', ['id' => $kompetisi])->row_array();
		...
		$where_kontingen = '';
		if($kontingen != 'semua') {
			$where_kontingen = "AND (kontingen_biru = '$kontingen' OR kontingen_merah = '$kontingen')";
		}
    ...
    $data_jadwal_pertandingan = $this->db->query("SELECT
                                                  ...
                                                  WHERE id_kompetisi = '$kompetisi'
																									AND status_selesai_pertandingan = 1
																									$where_gelanggang
																									$where_kontingen
                                ")->result_array();
    ...
    $this->load->view('laporan/cetak/laporan_perolehan_nilai', $data);
  }
    ...
    $data_jadwal_pertandingan = $this->db->query("SELECT
	    ...
	    // Apply style header yang telah kita buat tadi ke masing-masing kolom header
			...
			$sheet->setCellValue('B7', 'No Partai');
			$sheet->setCellValue('C7', 'Pesilat');
			$sheet->setCellValue('D7', 'Kontingen');
			$sheet->setCellValue('E7', 'Hukuman');
	    ...
	    $sheet->getStyle('E7')->applyFromArray($style_col);
			...
			foreach ($data_perolehan_nilai as $key => $data) {
				...
				$sheet->setCellValue('B'.$x, $data['no_partai']);
				$sheet->setCellValue('C'.$x, $data['nama_pesilat']);
				$sheet->setCellValue('D'.$x, $data['kontingen']);
				$sheet->setCellValue('E'.$x, $data['hukuman']);
				$sheet->setCellValue('F'.$x, $data['nilai']);

				// Apply style row yang telah kita buat tadi ke masing-masing baris (isi tabel)
      	$sheet->getStyle('A'.$x)->applyFromArray($style_row_center);
      	$sheet->getStyle('B'.$x)->applyFromArray($style_row_center);
      	$sheet->getStyle('C'.$x)->applyFromArray($style_row_left);
		...
		$filename = 'laporan-perolehan-nilai-' .strtolower(str_replace(' ', '-', $data_kompetisi['kompetisi']));
