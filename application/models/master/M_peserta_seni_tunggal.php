...
class M_peserta_seni_tunggal extends CI_Model {
  public function get_kelas_tanding(){
    return $this->db->query("SELECT * FROM ms_kelas_tanding ORDER BY id ASC")->result_array();
  }

  public function get_row_data($id){
    return $this->db->query("SELECT a.* FROM ms_peserta_seni_tunggal a WHERE id = '$id'")->row_array();
  }
  ...
  public function tambah(){
    ...
    $row_ktgn = $this->db->get_where('ms_kontingen', array('id' => $this->input->post('id_kontingen')))->row_array();
      ...
      'waktu' => date('H:i:s'),
		...
		$this->db->trans_begin();
		$this->db->insert('ms_peserta_seni_tunggal', $data);
  ...
  public function edit(){
    ...
    $peserta = $this->db->get_where('ms_peserta_seni_tunggal', ['id' => $id_peserta])->row_array();
    ...
    $this->db->where('id', $id_peserta);
		$this->db->update('ms_peserta_seni_tunggal', $data);
		$this->db->trans_complete();
  ...
  public function hapus(){
    ...
    $peserta = $this->db->get_where('ms_peserta_seni_tunggal', ['id' => $id])->row_array();
    ...
    $this->db->trans_begin();
		$this->db->where('id', $id);
		$this->db->delete('ms_peserta_seni_tunggal');
		$this->db->trans_complete();
  ...
  public function get_data(){
    ...
    $data = $this->db->query("SELECT a.*, b.kategori FROM ms_peserta_seni_tunggal a LEFT JOIN ms_kompetisi b ON a.id_kompetisi = b.id WHERE 1=1 $where ORDER BY id DESC")->result_array();
    return $data;
  }
  ...
  public function detail_data(){
    $id = $this->input->get('id');
    $data = $this->db->query("SELECT a.* FROM ms_peserta_seni_tunggal a
  ...
  public function import_excel(){
    ...
    if(isset($_FILES['file_excel']['name']) && in_array($_FILES['file_excel']['type'], $file_mimes)) {
          ...
          return $this->db->insert_batch('ms_peserta_seni_tunggal', $data);
      }
  }
}
