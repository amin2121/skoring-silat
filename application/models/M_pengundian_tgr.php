...
class M_pengundian_tgr extends CI_Model {
  public function undi(){
    ...
    $kategori_tanding = $this->input->get('kategori_tanding');

    $cek_peserta_undi = $this->db->query("SELECT
                                          a.*
                                          FROM ms_pengundian_tgr a
    ...
    if (count($cek_peserta_undi) <= 0) {
      ...
      if (count($cek_peserta) > 0) {
        ...
        $this->db->trans_begin();
        $this->db->insert_batch('ms_pengundian_tgr', $data_undian);
  ...
  public function tambah(){
		$data = [
      'pengundian_tgr' => $this->input->post('pengundian_tgr')
		];

		$this->db->trans_begin();
		$this->db->insert('ms_pengundian_tgr', $data);
		$this->db->trans_complete();
  ...
  public function edit(){
    ...
    $this->db->update('ms_pengundian_tgr', $data);
		$this->db->trans_complete();
  ...
  public function hapus(){
    ...
    $this->db->trans_begin();
		$this->db->where('id', $id);
		$this->db->delete('ms_pengundian_tgr');
		$this->db->trans_complete();
  ...
  public function get_data(){
    ...
    $data = $this->db->query("SELECT
                              a.*, b.nama_lengkap, b.golongan, b.kategori_tanding, b.kontingen
                              FROM ms_pengundian_tgr a
                              INNER JOIN ms_peserta b ON a.id_peserta = b.id
                              WHERE 1=1
                              $where
  ...
  public function get_data_row($id){
    return $this->db->get_where('ms_pengundian_tgr', array('id' => $id))->row_array();
  }
}
