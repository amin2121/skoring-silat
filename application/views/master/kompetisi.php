    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title">Data Kompetisi</h3>
            <div class="panel-elements pull-right">
              <a href="<?php echo base_url(); ?>master/kompetisi/view_tambah"><button class="btn btn-sm btn-primary"><i class=""></i>Tambah</button></a>
            </div>
        </div>
...
<script>
    ...
    function get_data() {
        ...
        $.ajax({
            url: '<?= base_url('master/kompetisi/get_data') ?>',
            method: 'GET',
            data: {search},
            dataType: 'json',
                  ...
                  for(const item of res) {
                    tr += `
                      <tr>
                          ...
                          <td class="text-center">${item.kategori == null ? '' : item.kategori.toUpperCase()}</td>
                          <th class="text-center">${item.status}</th>
                          <td class="text-center">
                              <div class="btn-group">
                                  <button class="btn btn-warning btn-sm" type="button" onclick="ubah_status(${item.id})" data-toggle="tooltip" data-original-title="Status"><i class="fa fa-check"></i></button>
    ...
    function ubah_status(id) {
      let confirmUbahStatus = confirm('Apakah Anda Ingin Mengubah Status Kompetisi')
      if(confirmUbahStatus) {
        $.ajax({
          url: '<?= base_url('master/kompetisi/ubah_status') ?>',
          method: 'POST',
          data: {id},
          dataType: 'json',
          success: function (res) {
    ...
    function edit_data(id) {
      window.location.href = '<?php echo base_url(); ?>master/kompetisi/view_edit/'+id
    }
    ...
    function hapus(){
      ...
      $.ajax({
        url: '<?php echo base_url('master/kompetisi/hapus') ?>',
        method: 'GET',
        data: {id},
        dataType: 'json',
        success: function (res) {
