...
<div class="app-heading-container app-heading-bordered bottom">
    <ul class="breadcrumb">
        <li class="active">Jadwal Partai Seni Tunggal</li>
    </ul>
</div>
<!-- END PAGE HEADING -->
    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title">Tambah Jadwal Partai Seni Tunggal</h3>
        ...
        <div class="panel-body">
          <form class="form-horizontal" method="POST" action="<?php echo base_url(); ?>master/jadwal_partai_seni_tunggal/tambah" enctype="multipart/form-data">
            <div class="form-group">
                <label class="col-md-2 control-label">Kompetisi</label>
                <div class="col-md-10">
                    <select class="form-control" name="id_kompetisi" id="select-kompetisi" onchange="ubah_kategori_by_kompetisi(); kontingen_result(); peserta_result();">
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                  <select class="form-control" name="peserta" id="nama_lengkap"></select>
                </div>
                <label class="col-md-2 control-label">Kontingen Pesilat</label>
                <div class="col-md-4">
                  <select class="form-control" name="kontingen" id="kontingen"></select>
                </div>
            </div>
            <div class="form-group">
...
<div class="modal fade" id="modal-import-data" tabindex="-1" role="dialog" aria-labelledby="modal-warning-header">
  <div class="modal-dialog modal-info" role="document">
      ...
      <div class="modal-content">
          ...
          <form class="" action="<?php echo base_url('master/jadwal_partai_seni_tunggal/import_excel') ?>" method="post" enctype="multipart/form-data">
            <div class="modal-body">
              <div class="form-group">
...
<script>
    ...
    function kontingen_result() {
        ...
        $.ajax({
            url: '<?= base_url('master/jadwal_partai_seni_tunggal/kontingen_result') ?>',
            method: 'POST',
            data: {id_kompetisi},
            dataType: 'json',
    ...
    function peserta_result() {
        let id_kompetisi = $(`#select-kompetisi`).val()

        $.ajax({
            url: '<?= base_url('master/jadwal_partai_seni_tunggal/peserta_result') ?>',
            method: 'POST',
            data: {id_kompetisi},
            dataType: 'json',
            success: function (res) {
                ...
                if(res.length > 0) {
                  for(const item of res) {
                    option += `<option value="${item.id}">${item.nama_lengkap}</option>`
                  }
                }

                $(`#nama_lengkap`).html(option)
            }
        })
    }
    ...
    function get_data() {
        ...
        $.ajax({
            url: '<?= base_url('master/jadwal_partai_seni_tunggal/get_data') ?>',
            method: 'GET',
            data: {search},
            dataType: 'json',
            success: function (res) {
    ...
    function edit_data(id) {
      window.location.href = '<?php echo base_url(); ?>master/jadwal_partai_seni_tunggal/view_edit/'+id
    }
    ...
    function hapus(){
      ...
      $.ajax({
        url: '<?php echo base_url('master/jadwal_partai_seni_tunggal/hapus') ?>',
        method: 'GET',
        data: {id},
        dataType: 'json',
        success: function (res) {
